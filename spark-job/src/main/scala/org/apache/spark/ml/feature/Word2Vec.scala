...
private[feature] trait Word2VecBase extends Params
  with HasInputCol with HasOutputCol with HasWeightCol
...
final class Word2Vec @Since("1.4.0")(
  ...
  /**
    ...
    *         增加一个是否带有权重列的判断
    * @param dataset      an rdd of input sentences
    * @param hasWeightCol indicate whether to attach additional weight to word sequence in input
    * @return
    */
  def fit(dataset: Dataset[_], hasWeightCol: Boolean = false): Word2VecModel =
    if (hasWeightCol) { // 有权重
      transformSchema(dataset.schema, logging = true)
      val input = dataset.select($(inputCol), $(weightCol)).rdd
        .map(row => (row.getAs[Seq[String]](0), row.getAs[Float](1)))

      val wordVectors = new feature.WeightedWord2Vec()
        .setLearningRate($(stepSize))
        .setMinCount($(minCount))
        .setNumIterations($(maxIter))
        .setNumPartitions($(numPartitions))
...
class Word2VecModel private[ml](
  ...
  /**
    ...
    *
    * @param dataset, a DF(PHONE_ID_COLUMN, TEXT_COLUMN, WEIGHT_COLUMN)
    * @param hasWeightCol
    * @return
    */
  def myTransform(dataset: Dataset[_], hasWeightCol: Boolean = false): DataFrame =
    if (hasWeightCol) {

      val PHONE_ID_COLUMN = "PHONE_ID"
      ...
      val weightSumPerWordPerId = dataset.select(PHONE_ID_COLUMN, $(inputCol), $(weightCol))
          .rdd
        .flatMap { row =>
          val id = row.getAs[String](PHONE_ID_COLUMN)
          val weight = row.getAs($(weightCol)).toString.toFloat
          row.getAs[Seq[String]]($(inputCol)).map(word => (id, word, weight))
        }
        .toDF(Array(PHONE_ID_COLUMN, TEXT_COLUMN, WEIGHT_COLUMN): _*)
        .groupBy(PHONE_ID_COLUMN, TEXT_COLUMN)
        .agg(sum(WEIGHT_COLUMN) as "weight_sum")

      weightSumPerWordPerId.show(5)
      ...
      weightSumPerWordPerId.rdd.map { row =>
        val id = row.getAs[String](PHONE_ID_COLUMN)
        val word = row.getAs[String](TEXT_COLUMN)
        val weight = row.getAs[Double]("weight_sum").toFloat
        id -> (word, weight)
      }
        .groupByKey()       // 按用户分组
        .mapValues { iter =>
          if (iter.isEmpty) {
            ...
            val sum = Vectors.zeros(d)
            iter.foreach { case (word, weight) =>
              bVectors.value.get(word).foreach { v =>
                BLAS.axpy(weight, v, sum)
              }
            }

            BLAS.scal(1.0 / iter.size, sum)
            //          BLAS.scal(1.0 / weights.sum, sum)

            sum
          }
        }
        .toDF(Array(PHONE_ID_COLUMN, $(outputCol)): _*)
    } else {
      transform(dataset)
    }
