...
import org.apache.spark.mllib.clustering.{GaussianMixture => MLlibGM, GaussianMixtureModel => MLlibGMM}
...
class GaussianMixture @Since("2.0.0") (
  ...
  /** @author lulin **/
  def myFit(dataset: Dataset[_], iniGaussianModel: MLlibGMM): GaussianMixtureModel = {
    transformSchema(dataset.schema, logging = true)
    val rdd: RDD[OldVector] = dataset.select(col($(featuresCol))).rdd.map {
      case Row(point: Vector) => OldVectors.fromML(point)
    }
    ...
    val algo = new MLlibGM()
      ...
      .setSeed($(seed))
      .setConvergenceTol($(tol))
      .setInitialModel(iniGaussianModel)
    val parentModel = algo.run(rdd)
    val gaussians = parentModel.gaussians.map { case g =>
      new MultivariateGaussian(g.mu.asML, g.sigma.asML)
    }
